...
- `findIntersections(segments)`: All intersecting pairs. Runs in
  $O((n+k) \log n)$ time for $k$ reported pairs.

- `findCrossings(segments)`: All crossing pairs, in the same
  $O((n+k) \log n)$ time.

- `findInteriorIntersections(segments)`: All pairs whose interiors intersect —
  the crossing pairs plus the pairs overlapping along a segment of positive
  length — in the same $O((n+k) \log n)$ time.

- `detectIntersections(segments)` / `detectCrossings(segments)` /
  `detectInteriorIntersections(segments)`: Return `True` as soon as one such
...
- `smallestEnclosingDisk(points)`: Returns the unique smallest closed
...
- `findEmptyTriangles(points)`: Returns every empty
  [`Triangle`](shapes.md#triangle), in $O(n^2 \log n + k)$ time for `n`
  distinct points and `k` triangles.

- `findEmptyQuadrilaterals(points)`: Returns every empty quadrilateral as a
  [`Polygon`](shapes.md#polygon) — simple, and possibly non-convex.

- `findEmptyConvexQuadrilaterals(points)`: Returns the convex ones only, as
...
- a `Point`, reporting only the polygons with it as a **vertex**. It need not be
...
- `closestPair(points)`: Returns a [`Segment`](shapes.md#segment) joining two of
...
- `regularizedUnionOf(shapes, simple_boundaries=False)`: Returns the regularized
  ...
  one of the six bounded region types — `Triangle`, `Rectangle`, `Convex`,
  `Polygon`, `PolygonWithHoles` or `PolygonSet`; a set contributes its
  components. Set `simple_boundaries` when no piece has two boundary edges
  overlapping each other (always true of a `Triangle`, a `Rectangle` and a
  `Convex`, true of a `Polygon` that meets its simplicity precondition, and true
  of a `PolygonWithHoles` exactly when it carries no slit) to take the faster
