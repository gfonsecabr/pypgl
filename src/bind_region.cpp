...
void bind_region(nb::module_ &m) {
    ...
    PGL_BIND_LATTICE_POINTS(cls, PolygonWithHoles,
                            ...
                            "point on an edge is a point of the shape. A hole is not part of the region, and "
                            "neither are the points inside one; the points on a hole's boundary are, as any "
                            "boundary point is.");
    PGL_BIND_AS_BIT_MATRIX(cls, PolygonWithHoles);
